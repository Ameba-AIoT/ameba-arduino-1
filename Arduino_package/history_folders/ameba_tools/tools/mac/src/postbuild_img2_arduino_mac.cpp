...
int main(int argc, char *argv[]) {
    ...
    // 6. fulfill header
    cmdss.clear();
    cmdss << "./tools/mac/pick " << sram_start << " " << sram_end << " ram_2.bin ram_2.p.bin body+reset_offset+sig";
    getline(cmdss, cmd);
    cout << cmd << endl;
    system(cmd.c_str());

    cmdss.clear();
    cmdss << "./tools/mac/pick " << sram_start << " " << sram_end << " ram_2.bin ram_2.ns.bin body+reset_offset";
    getline(cmdss, cmd);
    cout << cmd << endl;
    system(cmd.c_str());
    ...
    if (has_sdram) {
        cmdss.clear();
        cmdss << "./tools/mac/pick " << sdram_start << " " << sdram_end << " sdram.bin ram_3.p.bin body+reset_offset";
        getline(cmdss, cmd);
        cout << cmd << endl;
        system(cmd.c_str());
    }
    ...
    cmd = "./tools/mac/padding 44k 0xFF ram_1.p.bin";
    cout << cmd << endl;
    system(cmd.c_str());
    ...
    // 9. add checksum
    cmd = "./tools/mac/checksum ota.bin";
    cout << cmd << endl;
    system(cmd.c_str());
