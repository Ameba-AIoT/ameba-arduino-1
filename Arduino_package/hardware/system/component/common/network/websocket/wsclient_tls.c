...
int ws_setsockopt_rcvtimeo(wsclient_context* wsclient, int recv_timeout)
{
	int ret = 0;
	...
	ret = setsockopt(wsclient->sockfd, SOL_SOCKET, SO_RCVTIMEO, &timeout, sizeof(timeout));
#else	//lwip 1.4.1
	ret = setsockopt(wsclient->sockfd, SOL_SOCKET, SO_RCVTIMEO, &recv_timeout, sizeof(recv_timeout));
#endif
	return ret;
}

int ws_setsockopt_sndtimeo(wsclient_context* wsclient, int send_timeout)
{
	int ret = 0;
#if defined(LWIP_SO_SNDRCVTIMEO_NONSTANDARD) && (LWIP_SO_SNDRCVTIMEO_NONSTANDARD == 0)	//lwip 2.0.2
	struct timeval timeout;
	...
	ret = setsockopt(wsclient->sockfd, SOL_SOCKET, SO_SNDTIMEO, &timeout, sizeof(timeout));
#else	//lwip 1.4.1
	ret = setsockopt(wsclient->sockfd, SOL_SOCKET, SO_SNDTIMEO, &send_timeout, sizeof(send_timeout));
#endif
	return ret;
}
