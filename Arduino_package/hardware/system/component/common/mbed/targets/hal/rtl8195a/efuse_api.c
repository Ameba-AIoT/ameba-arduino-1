...
extern VOID  ReadEfuseContent1(OUT u8 *pContent);
extern u8 WriteEfuseContent1(IN u8 CodeWordNum, IN u8 WordEnable, IN u8 *pContent);
extern VOID ReadEOTPContent(OUT u8 *pContent);
VOID ReadEOTPContent_ex(OUT u8 *pContent);
extern u32  WriteEOTPContent(IN u8 *pContent);
extern u32 EOTPChkContent(IN u8 * pContent);
extern u32  WriteKEY1(IN u8 *pContent);
extern u32  WriteKEY2(IN u8 *pContent);
...
int  efuse_mtp_write(uint8_t *data, uint8_t len)
{
	...
	switch (len_high){
		case 0:
			WriteEfuseContent1(0, word_enable, data);
			break;
			
		case 1:
			WriteEfuseContent1(0, 0xf, data);
			WriteEfuseContent1(1, word_enable, data+8);	
			break;

		case 2:
			WriteEfuseContent1(0, 0xf, data);
			WriteEfuseContent1(1, 0xf, data+8);
			WriteEfuseContent1(2, word_enable, data+8);
			break;

		case 3:
			WriteEfuseContent1(0, 0xf, data);
			WriteEfuseContent1(1, 0xf, data+8);
			WriteEfuseContent1(2, 0xf, data+16);
			WriteEfuseContent1(3, word_enable, data+24);
			break;
		...
		case 4:
			...
			WriteEfuseContent1(1, 0xf, data+8);
			WriteEfuseContent1(2, 0xf, data+16);
			WriteEfuseContent1(3, 0xf, data+24);
	}		
	return len;
}
...
int efuse_otp_write(u8 address, u8 len, u8 *buf)
{
	...
	result = WriteEOTPContent(content);
