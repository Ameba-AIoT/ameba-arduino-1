...
HeapRegion_t xHeapRegions[] =
{
     ...
     * These regions are assigned values at runtime before OS start.
     */
    { NULL, 0 },
    { NULL, 0 },
#endif
...
#ifdef ARDUINO_SDK
/**
 * Add a heap region if there is available region in xHeapRegions.
 * This API can only be called before OS start.
 *
 * @param[in] addr The address of the region
 * @param[in] size The size of the region
 * @return Return 0 if success. Return 0 if fail.
 */
int vPortAddHeapRegion(uint8_t *addr, size_t size) {
    int i;
    if(pxEnd == NULL) {
        for (i = 0; i < ( sizeof(xHeapRegions) / sizeof(xHeapRegions[0]) ) - 1; i++) {
            if (xHeapRegions[i].pucStartAddress == NULL && xHeapRegions[i].xSizeInBytes == 0) {
                xHeapRegions[i].pucStartAddress = addr;
                xHeapRegions[i].xSizeInBytes = size;
                return 0;
            }
        }

        // Cannot find available region
        return -1;
    } else {
        // OS has started.
        return -1;
