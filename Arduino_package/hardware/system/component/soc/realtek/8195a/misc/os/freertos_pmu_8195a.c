...
void register_sleep_callback_by_module( unsigned char is_pre_sleep, freertos_sleep_callback sleep_cb, uint32_t module ) {
    uint32_t i, j;
    for (i=0; i<psm_dd_hook_info_size; i++) {
        if ( BIT(psm_dd_hook_infos[i].nDeviceId) == module ) {
            if (is_pre_sleep) {
                psm_dd_hook_infos[i].sleep_hook_fun   = (PSM_HOOK_FUN)sleep_cb;
                psm_dd_hook_infos[i].sleep_param_ptr  = NULL;
            } else {
                psm_dd_hook_infos[i].wakeup_hook_fun  = (PSM_HOOK_FUN)sleep_cb;
                psm_dd_hook_infos[i].wakeup_param_ptr = NULL;
            }
            break;
        }
    ...
    if (i == psm_dd_hook_info_size) {
        for (j=0; j<32; j++) {
            if (BIT(j) & module) {
                psm_dd_hook_infos[psm_dd_hook_info_size].nDeviceId        = j;
                if (is_pre_sleep) {
                    psm_dd_hook_infos[psm_dd_hook_info_size].sleep_hook_fun   = (PSM_HOOK_FUN)sleep_cb;
                    psm_dd_hook_infos[psm_dd_hook_info_size].sleep_param_ptr  = NULL;
                } else {
                    psm_dd_hook_infos[psm_dd_hook_info_size].wakeup_hook_fun  = (PSM_HOOK_FUN)sleep_cb;
                    psm_dd_hook_infos[psm_dd_hook_info_size].wakeup_param_ptr = NULL;
                }
                psm_dd_hook_info_size++;
                break;
            }
...
void register_pre_sleep_callback( freertos_sleep_callback pre_sleep_cb ) {
    register_sleep_callback_by_module(1, pre_sleep_cb, 0x00010000);
}

void register_post_sleep_callback( freertos_sleep_callback post_sleep_cb ) {
    register_sleep_callback_by_module(0, post_sleep_cb, 0x00010000);
}
