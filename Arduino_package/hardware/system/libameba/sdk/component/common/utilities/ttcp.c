...
sys_thread_t ThreadTx = NULL;
sys_thread_t ThreadRx = NULL;
#endif
...
tcp_send_loop(struct ttcp *ttcp)
{
	...
	if(cnt == 0)
		printk("TTCP [%p]: snd_queuelen is not empty !!!\n", ttcp);
}
#endif
...
void tcp_connect_timer_cb(void* ctx)
{
...
tcp_start(struct ttcp* ttcp)
{
  ...
  #if USE_TIMER_MSG
	ttcp->thdl_msg = xTimerCreate( ( char const * ) "MsgTimer", MSG_TIME, pdTRUE, ttcp, tcp_timer_msg );
	xTimerStart(ttcp->thdl_msg, 1000);
  #endif
	if (ttcp->mode == TTCP_MODE_TRANSMIT) {
		tcp_err(ttcp->tpcb,  tcp_conn_err_cb);
		tcp_recv(ttcp->tpcb, tcp_recv_cb);
		tcpip_callback(tcp_connect_timer_cb, ttcp->tpcb);
...
int
ttcp_start(struct ip_addr addr, uint16_t port, void *opaque, ttcp_done_cb_t *done_cb, 
...
//---------------------------------------------------------------------
#if (ACTION_MODE == ACTION_THREAD)
void ttcp_thread_rx(void *param)
{
	struct ttcp_cfg *cfg = (struct ttcp_cfg*)param;
	
	printk("TTCP RX thread start");
	ttcp_start(cfg->addr, cfg->port, NULL, NULL, cfg->mode, cfg->nbuf, cfg->buflen, cfg->udp, cfg->verbose, cfg->tos);
	if(param)
		free(param);
	ThreadRx = NULL;
	printk("TTCP RX thread exit");
	vTaskDelete(NULL);
}

void ttcp_thread_tx(void *param)
{
	struct ttcp_cfg *cfg = (struct ttcp_cfg*)param;
	
	printk("TTCP TX thread start");
	ttcp_start(cfg->addr, cfg->port, NULL, NULL, cfg->mode, cfg->nbuf, cfg->buflen, cfg->udp, cfg->verbose, cfg->tos);
	if(param)
		free(param);
	ThreadTx = NULL;
	printk("TTCP TX thread exit");
	vTaskDelete(NULL);
}

int ttcp_thread(struct ip_addr addr, uint16_t port, void *opaque, ttcp_done_cb_t *done_cb, 
				uint8_t mode, uint32_t nbuf, uint32_t buflen, uint8_t udp, uint8_t verbose, uint8_t tos)
{
	struct ttcp_cfg *pTTCPcfg;
	
	if(mode == TTCP_MODE_RECEIVE){
		if(ThreadRx){
			printk("[%s] TTCP RX task has created.", __FUNCTION__);
			return -1;
		}
	}
	if(mode == TTCP_MODE_TRANSMIT){
		if(ThreadTx){
			printk("[%s] TTCP TX task has created.", __FUNCTION__);
			return -1;
		}
	}
	pTTCPcfg = malloc(sizeof(struct ttcp_cfg));
	if(pTTCPcfg == NULL){
		printk("[%s] Alloc TTCP cfg failed", __FUNCTION__);
		return -1;
	}
	pTTCPcfg->addr = addr;
	pTTCPcfg->port = port;
 	pTTCPcfg->nbuf = nbuf;
 	pTTCPcfg->mode = mode;
 	pTTCPcfg->done_cb = done_cb;
 	pTTCPcfg->opaque = opaque;
 	pTTCPcfg->udp = udp;
 	pTTCPcfg->verbose = verbose;
 	pTTCPcfg->buflen = buflen;
 	pTTCPcfg->tos = tos;
	
	if(mode == TTCP_MODE_RECEIVE)
	{
		printk("%s: ttcp_thread_rx", __FUNCTION__);
		if((ThreadRx = sys_thread_new("ttcp_thread_rx", ttcp_thread_rx, pTTCPcfg, STACKSIZE, tskIDLE_PRIORITY + 1 )) == NULL){
//		if(xTaskCreate(ttcp_thread_rx, "ttcp_thread_rx", STACKSIZE, NULL, tskIDLE_PRIORITY + 1, NULL) != pdPASS){
			printk("%s xTaskCreate ttcp_thread_rx failed", __FUNCTION__);
			return -1;
		}
	}else
	{
		printk("%s: ttcp_thread_tx", __FUNCTION__);
		if((ThreadTx = sys_thread_new("ttcp_thread_tx", ttcp_thread_tx, pTTCPcfg, STACKSIZE, tskIDLE_PRIORITY + 2 )) == NULL){
//		if(xTaskCreate(ttcp_thread_tx, "ttcp_thread_tx", STACKSIZE, NULL, tskIDLE_PRIORITY + 2, NULL) != pdPASS){	// if priority + 1, pcb->snd_queuelen in tcp_send_loop() will not be 0.
			printk("%s xTaskCreatet ttcp_thread_tx failed", __FUNCTION__);
			return -1;
		}
	}
	return 0;
...
#endif // #if (ACTION_MODE == ACTION_THREAD)
...
void cmd_ttcp(int argc, char **argv)
{
	...
	if(argc == 3 && strcmp(argv[1], "r") == 0){
		...
		buflen = atoi(argv[2]);
	  #if (ACTION_MODE == ACTION_THREAD)
		ret = ttcp_thread(addr, port, NULL, NULL, mode, nbuf, buflen, udp, verbose, tos);
	  #else
		ret = ttcp_start(addr, port, NULL, NULL, mode, nbuf, buflen, udp, verbose, tos);
	  #endif
	}else if(argc == 4 && strcmp(argv[1], "r") == 0 && strcmp(argv[2], "u") == 0){
		// receive
		buflen = atoi(argv[3]);
		udp = 1;
	  #if (ACTION_MODE == ACTION_THREAD)
		...
		ret = ttcp_start(addr, port, NULL, NULL, mode, nbuf, buflen, udp, verbose, tos);
	  #endif
	}else if(argc == 6 && strcmp(argv[1], "t") == 0 && strcmp(argv[2], "u") == 0){
		// transmit
		mode = TTCP_MODE_TRANSMIT;
		udp = 1;
		addr.addr = inet_addr(argv[5]);
		if (!addr.addr) {
			printk("%s: address type error!", __FUNCTION__);
			return;
		}
		...
		ret = ttcp_start(addr, port, NULL, NULL, mode, nbuf, buflen, udp, verbose, tos);
	  #endif
	}else if(argc == 6 && (strcmp(argv[1], "rt") == 0 || strcmp(argv[1], "tr") == 0) && strcmp(argv[2], "u") == 0){
		// receive
		buflen = atoi(argv[3]);
		udp = 1;
	  #if (ACTION_MODE == ACTION_THREAD)
