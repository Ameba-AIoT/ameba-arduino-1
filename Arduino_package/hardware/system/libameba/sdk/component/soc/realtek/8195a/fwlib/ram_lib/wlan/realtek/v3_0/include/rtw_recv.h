...
/* struct phy_info must be same with ODM ODM_PHY_INFO_T */
...
struct phy_info
{
	...
	s8		RxMIMOSignalQuality[MAX_PATH_NUM_8195A];	//per-path's EVM
	u8		RxMIMOEVMdbm[MAX_PATH_NUM_8195A]; 		//per-path's EVM dbm
	u8		RxMIMOSignalStrength[MAX_PATH_NUM_8195A];// in 0~100 index
	u16		Cfo_short[MAX_PATH_NUM_8195A]; 			// per-path's Cfo_short
	u16		Cfo_tail[MAX_PATH_NUM_8195A];			// per-path's Cfo_tail

	s8		RxPower; // in dBm Translate from PWdB
	s8		RecvSignalPower;// Real power in dBm for this packet, no beautification and aggregation. Keep this raw info to be used for the other procedures.
	u8		BTRxRSSIPercentage;
	u8		SignalStrength; // in 0-100 index.
	s8		RxPwr[MAX_PATH_NUM_8195A];				//per-path's pwdb

	u8		RxSNR[MAX_PATH_NUM_8195A];				//per-path's SNR
	u8		BandWidth;
	u8		btCoexPwrAdjust;
};
#else
